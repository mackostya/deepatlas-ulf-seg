"""
Implementation of this model is borrowed and modified from
https://github.com/black0017/MedicalZooPytorch/blob/master/lib/medzoo/Vnet.py
---
Implementation of this model in MedicalZooPytorch is borrowed and modified
...
from src.vnet.base_model import BaseModel
...
class VNet(BaseModel):
    ...
    def __init__(self, elu=True, in_channels=1, classes=4, use_atlas=False):
    ...
    def forward(self, x, atlas=None):
        ...
        if atlas is not None:
            atlas = self.atlas_encoder(atlas, interpolate_to=8)
            out256 = out256 + atlas
...
if __name__ == "__main__":
    ...
    model = VNetLight(in_channels=1, classes=1)
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(
            model, (1, 128, 128, 128), as_strings=True, print_per_layer_stat=False, verbose=True
        )
        print("{:<30}  {:<8}".format("Computational complexity: ", macs))
        print("{:<30}  {:<8}".format("Number of parameters: ", params))
    model = VNetLight(in_channels=1, classes=9)
    with torch.cuda.device(0):
        macs, params = get_model_complexity_info(
            model, (1, 128, 128, 128), as_strings=True, print_per_layer_stat=False, verbose=True
        )
